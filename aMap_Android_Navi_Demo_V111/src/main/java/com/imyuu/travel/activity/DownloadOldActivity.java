...
import com.imyuu.travel.ui.CustomProgressDialog;
...
public class DownloadOldActivity extends DownloadActivity{
    ...
    public DownloadOldActivity(Activity activity)
    {
    ...
    public String download(final String scenicId, Handler handler) {
        ...
        String url = Config.OLDMAP_SERVER_ADDR + "scenicm"+scenicId + ".zip";
        //开始下载文件
        File saveFile = new File(Config.Map_FILEPATH, scenicId + ".zip");
        if(saveFile.exists())
            saveFile.delete();
         Log.d("DownloadOldActivity",url);

        download(url,saveFile, handler);
        return saveFile.getAbsolutePath();
    }

    public void download(final Activity activity, final String sncId) {
        this.activity = activity;
        this.scenicId = sncId;
        ...
        progressDialog = new CustomProgressDialog(activity,"正在努力下载地图");
        progressDialog.show();
        //下载景区详细资料
        new Thread() {
            @Override
            public void run() {
                ...
                int result = HttpOldUtil.downFile(Config.OLDMAP_SERVER_ADDR + "m" + scenicId,
                        Config.Map_FILEPATH, ConstantsOld.SCENIC + scenicId + ConstantsOld.ALL_SCENIC_ZIP);
    ...
    private void parseOldMap(final String scenicId)
    {
        String scenicName = "";
        String imagepath = "";
        try {
            Log.e("start upZip", "start upZip");
            ZipUtil.upZipFile(new File(Config.Map_FILEPATH + scenicId + ConstantsOld.ALL_SCENIC_ZIP), Environment.getExternalStorageDirectory() + "/" + ConstantsOld.SCENIC_ROUTER_FILE_PATH);
            ...
            if (TextUtils.isEmpty(json)) {
                ...
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        if (!TextUtils.isEmpty(scenicMapId)) {
                            if (scenicMapDataHelper.getModelById(scenicMapId) == null) {
                                ...
                                scenicName = ScenicMapOldModel.getScenicName();
                                            ...
                                            for (int k = 0; k < jsonArrayLG.length(); k++) {
                                                ...
                                                if (!TextUtils.isEmpty(recommendLinesectionguideId)) {
                                                    if (recommendLinesectionguideDataHelper.getModelById(recommendLinesectionguideId) == null) {
                                                        ...
                                                        recommendLinesectionguideModel.setScenicId(scenicId);
                    ...
                    new Thread() {
                        ...
                        public void run() {
                            HttpOldUtil HttpOldUtil = new HttpOldUtil();
                            int result = HttpOldUtil.downFile(Config.OLDMAP_SERVER_ADDR + "scenicAdvert"+scenicId+".zip", Config.Map_FILEPATH, ConstantsOld.SCENIC_ADVERT + scenicId + ConstantsOld.ALL_SCENIC_ZIP);
                            Message message = new Message();
                            Bundle bundle = new Bundle();
                            bundle.putInt(ConstantsOld.API_MESSAGE_KEY, result);
                            message.setData(bundle);
                    ...
                    mapInfoModel.setScenicName(scenicName);
                    mapInfoModel.setScenicId(scenicId);
                    try {
                        imagepath = CityScenesJson.load(scenicId).getSmallImage();

                    } catch (Exception e) {
                      //  e.printStackTrace();
                        imagepath = ScenicAreaJson.load(scenicId).getSmallImage();
                    }
                    mapInfoModel.setImagePath(imagepath);
                    mapInfoModel.setCanNav("0");
    ...
    private Handler handlerAdvert = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (result) {
                ...
                case 0:
                    try {
                        ZipUtil.upZipFile(new File(Config.Map_FILEPATH + ConstantsOld.SCENIC_ADVERT + scenicId + ConstantsOld.ALL_SCENIC_ZIP), Config.UU_FILEPATH );
