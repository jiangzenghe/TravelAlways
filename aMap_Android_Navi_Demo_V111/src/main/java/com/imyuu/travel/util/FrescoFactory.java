...
// .setFadeDuration(fadeDuration)//fresco:fadeDuration="300"加载图片动画时间
                .setFailureImage(FrescoFactory.sErrorDrawable)//fresco:failureImage="@drawable/error"失败图
...
// .setOverlays(overlays)
                .setPlaceholderImage(FrescoFactory.sPlaceholderDrawable)//fresco:placeholderImage="@color/wait_color"占位图
...
// .setMinDecodeIntervalMs(intervalMs)//最小解码间隔（分位单位）
   ...
   public static void setDraweewithController(SimpleDraweeView view, String uri)
   {
       ImageRequest request = FrescoFactory.getImageRequest(view, uri);
       if (null != request) {
           view.setDrawingCacheEnabled(true);
           DraweeController draweeController = FrescoFactory.getPipelineControllder(request, view);
    ...
    public static ImageRequest getImageRequest(SimpleDraweeView view, String uri) {
        ...
        // .setSource(Uri uri)//设置图片地址
            ...
            Log.d("ImageRequest",uri);
        }
        return imageRequest;
    }

    public static  DraweeController getPipelineControllder(ImageRequest imageRequest, SimpleDraweeView view)
    {
